...
- **Уроки** берутся из MongoDB (`lessons`: `dayNum`, `tasks`).
...
```json
{
  ...
  "tasks": [
    {
      "text": "Задание 1...",
      "images": ["day1_task1_1.jpg", "day1_task1_2.jpg"]
    },
    {
      "text": "Задание 2...",
      "images": []
...
- `tasks` — список заданий на день.
  - `tasks[i].text` — текст задания.
  - `tasks[i].images` — имена файлов из папки `media/`.
...
- `next_unlock_at` — момент (UTC), когда откроется следующий день.
  ...
  - При завершении дня N бот выставляет **следующий день** на **08:00 следующего календарного дня по времени Europe/Moscow (МСК)**.
- `last_completed_at` — когда пользователь последний раз отмечал день как выполненный (UTC).

Поля для "выполнения по заданиям" внутри дня:

- `in_progress_day` — номер дня, который пользователь сейчас проходит (обычно `completed_day + 1`).
- `current_task` — индекс текущего задания (0-based).

### Ограничение "один день в сутки"

После выполнения дня **следующий день становится доступен только на следующий календарный день в 08:00 по московскому времени (МСК)**.
...
В репозитории есть пример: `lessons.example.json` (использует файлы из `media/`).
...
- Поля, которые обновляются: `dayNum`, `tasks`.

> Legacy-формат `{dayNum, text, images}` тоже поддерживается и будет автоматически
> преобразован в одно задание внутри `tasks`.
